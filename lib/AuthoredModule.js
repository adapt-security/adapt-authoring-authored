...
class AuthoredModule extends AbstractModule {
  ...
  async registerModule (mod, options = { accessCheck: true }) {
    ...
    mod.requestHook.tap((...args) => this.updateAuthor(mod, ...args))
   ...
   */
  async updateAuthor (mod, req) {
    const userId = req.auth.user._id.toString()

    if (req.apiData.modifying) {
      const _id = req.apiData.data._id
      if (_id) {
        const [originalDoc] = await mod.find({_id})
        const [config] = await mod.find({$and: [{_type: 'config'}, {_courseId: originalDoc._courseId}]})
        await mod.update({_id: config._id}, {courseLastUpdatedBy:userId}, {validate:false})
      }
    }
    if (req.method === 'POST' && req.apiData.modifying && !req.apiData.data.createdBy) {
      req.apiData.data.createdBy = userId
    }
  }
